				...
				<table class="table table-hover" id="inboxTable">
					...
					<tbody>
						...
						<tr @if($row->status == 0) class="table-dark text-dark" @endif>
							<td>{{ $row->id }}</td>
							<th class="text-capitalize">{{ $row->from_name }}</th>
							...
							<td><button class="btn btn-secondary btn-xs" id="viewInbox" data-id="{{ $row->id }}" data-token="{{ csrf_token() }}" >View</button> <a data-toggle="modal" data-target="#messageModal{{ $row->id }}" href="#" class="btn btn-xs btn-secondary">Reply</a></td>
						</tr>
						@endforeach
					</tbody>
				</table>
